    ...
    <div className="relative">
      <ChartFigure summary={summary} height={H}>
        {(W) => {
          ...
          const column = (W - PAD.left - PAD.right) / labels.length;

          // Running baseline per point so stacked bands sit on one another.
                  ...
                  {stacked ? (
                    <>
                      <path
                        ...
                        opacity={0.75}
                      />
                      {/* Surface-coloured gap keeps stacked fills legible. */}
                      <path
                        d={band.line}
                        fill="none"
                        stroke="var(--card)"
                    ...
                    /* Unstacked compares series, so lines only - overlapping
                       translucent fills would occlude each other. */
                    <path
                      d={band.line}
                      fill="none"
              ...
              {labels.map((label, index) => (
                <Fragment key={label}>
                  <text
                    x={x(index)}
                    y={H - 6}
                    /* Edge ticks are anchored, not centred, so the first and
                       last labels cannot bleed past the plot area. */
                    textAnchor={
                      index === 0
                        ? "start"
                  ...
                  {/* Hit target spans the whole column, far larger than a mark. */}
                  <rect
                    x={x(index) - column / 2}
                    y={PAD.top}
                    width={column}
